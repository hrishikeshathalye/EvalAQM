...
import sys
try:
    from datafile_fq import dic_fq
    from datafile_adaptive import dic_adaptive
except:
...
for iteration in range(2):
    ...
    if iteration == 0:
        dic = dic_fq
        data_file = "data_fq.txt"
        mos_file = "mos_file_fq.txt"
    else:
        dic = dic_adaptive
        data_file = "data_adaptive.txt"
...
for i, item in enumerate(y_axis):
    ...
    f.write(f"{item['max']}\n")   # y-range max
    f.write(f"{item['min']}\n")   # y-range min
    f.write(f"{int(dic_fq['x_values'][n - 1]) + 1}\n") # x-range max
f.close()
